...
# 默认的密度值（作为后备）
DEFAULT_CROWD_COUNT = ['20', '55', '90', '125', '160', '195']
...
def load_density_bins(config_path=None):
    """
    从配置文件加载自动计算的密度区间
    ...
    if os.path.exists(config_path):
        try:
            ...
            crowd_count = config['crowd_count']
            logger.info(f"Loaded density bins from {config_path}: {crowd_count}")

            # 验证bins
            if len(crowd_count) != 6:
                logger.warning(f"Expected 6 density bins, got {len(crowd_count)}. Using default.")
                return DEFAULT_CROWD_COUNT

            # 打印统计信息
            if 'statistics' in config:
                stats = config['statistics']
                logger.info(f"Dataset statistics - "
                          f"Mean: {stats.get('patch_mean', 'N/A'):.1f}, "
                          f"Std: {stats.get('patch_std', 'N/A'):.1f}, "
                          f"Range: [{stats.get('min_count', 'N/A')}, {stats.get('max_count', 'N/A')}]")

            return crowd_count

        except Exception as e:
            logger.warning(f"Failed to load density bins from {config_path}: {e}")
            logger.info(f"Using default density bins: {DEFAULT_CROWD_COUNT}")
            return DEFAULT_CROWD_COUNT
    else:
        logger.info(f"Density bins config not found at {config_path}")
        logger.info(f"Using default density bins: {DEFAULT_CROWD_COUNT}")
        return DEFAULT_CROWD_COUNT


# 全局加载密度bins（可以通过环境变量指定配置路径）
DENSITY_CONFIG_PATH = os.getenv('DENSITY_CONFIG_PATH', None)
crowd_count = load_density_bins(DENSITY_CONFIG_PATH)


@MODELS.register_module()
class CrowdCLIPDynamic(nn.Module):
    """
    支持动态密度区间的CrowdCLIP模型
    """
    def __init__(
            self,
            text_encoder_name,
            image_encoder_name,
            density_config_path=None,
            **kwargs,
    ) -> None:
        super().__init__()
        ...
        # 加载密度bins
        if density_config_path:
            self.crowd_count = load_density_bins(density_config_path)
        else:
            self.crowd_count = crowd_count  # 使用全局加载的值

        logger.info(f"CrowdCLIPDynamic initialized with density bins: {self.crowd_count}")
    ...
    def forward(self, images, phase='train'):
        if phase == 'train':
            ...
            for i in range(image_features.size()[0]):
                ...
                count.append(int(self.crowd_count[indices[0]]))

        else:
            new_x = images
            j = 0
            text1_inputs = torch.cat([
                clip.tokenize(f"The object is {p}") for p in person_exist
            ]).cuda()

            text2_inputs = torch.cat([
                clip.tokenize(f"The objects are {p}") for p in person_exist2
            ]).cuda()
            ...
            text_features_split = torch.split(text_features,
                                              [len(person_exist), len(person_exist2), len(self.crowd_count)])
            ...
            for i in range(images.size()[0]):
                ...
                if person_exist[indices[0]] == 'crowd':
                    ...
                    if (person_exist2[indices[0]] == 'human heads' or
                        person_exist2[indices[1]] == 'human heads' or
